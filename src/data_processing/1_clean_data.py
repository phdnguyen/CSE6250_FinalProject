...
contractions = pd.read_json(
    contractions_path, typ="series"
)  # getting the list of English contractions
...
def main():
    ...
    labeled_notes = label_source.merge(
        notes, how="left", left_on=["row_id"], right_on=["ROW_ID"]
    )

    # make another column for binary labels (i.e. only smoking vs. non smoking)

    labeled_notes["behavior_tobacco_binary"] = labeled_notes["behavior_tobacco"].apply(
    ...
    labeled_notes[["TEXT", "behavior_tobacco", "behavior_tobacco_binary"]].to_csv(
