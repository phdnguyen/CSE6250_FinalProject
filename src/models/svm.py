...
def main():
    ...
    MAX_SEQ_LENGTH = max(
        len(text.split()) for text in df["TEXT"]
    )  # maximum length of a note event
    MAX_FEATURES = (
        10000  # only consider the top 10000 most frequent words in the corpus
    )
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(
        tdidf_tensor, df["behavior_tobacco"].values, test_size=0.3, random_state=42
    )
